...
| **`sdd-run`** | ejecución | Ejecuta una spec de punta a punta en un worktree aislado desde el base actualizado. El checkout original puede seguir sucio: importa al branch sólo la spec, contrato, handoff y demás artefactos de entrada ligados a esa cadena, y deja el resto excluido e intacto. Después planifica contra el código real, implementa **con tests primero**, verifica cada criterio con su mecanismo declarado y termina en un **PR** con la spec como body + evidencia. |
| **`sdd-review-loop`** | pulido del PR | Encadena hasta N rondas autónomas de review y corrección sobre un PR existente: en cada ronda un subagente `reviewer` corre el `/code-review` nativo de Claude Code con `--comment` (en Pi, el skill `code-review` del package, que publica un review `COMMENT` por default), y si quedan hallazgos de correctness otro subagente `implementer` los corrige con la doctrina de remediación de `sdd-run`, verifica con los comandos del contrato (`.sdd/project.md`), pushea al branch del PR y resuelve los threads atendidos. El orquestador vive en la conversación principal y retiene solo conteos y claves por ronda. Corta por parada temprana, no convergencia o tope de 5 rondas, y deja un único comment resumen idempotente en el PR. Invocado pelado abre un wizard; con solo `<PR>` pregunta igual rondas, nivel, umbral y modelos; cualquier flag (`/sdd-review-loop <PR> [--rounds N] [--level L] [--fix-scope S] [--model M]`) lo saltea. Claude Code y Pi (en Pi `/skill:sdd-review-loop`, sin `--level`: su `code-review` no tiene niveles, y los modelos se heredan de la sesión salvo `--model`): exige `.sdd/project.md` y un PR abierto del mismo repo. |
...
| **`wait-pr`** | Comando `/wait-pr`; materializa el skill canónico `/skill:wait-pr` sin inyectar una invocación slash y falla cerrado si falta `code-review`. |
| **`subagent`** | Tool `subagent` para delegar tareas a subagentes con contexto aislado (hijos `pi --mode json -p --no-session`): modos single, parallel (hasta 8 tareas, 4 concurrentes) y chain (con `{previous}`), `agentScope` `package`/`user`/`project`/`all` (default `package`: los agentes bundleados `implementer`, `reviewer` y `scout` en `pi-extensions/subagent/agents/`) y `background: true` para recibir el resultado como mensaje `subagent-result` sin bloquear la sesión. Comando `/subagents` para listar los jobs (`sa-N`, agente, estado, duración) y `/subagents abort <id>`. Al cerrar la sesión mata los hijos vivos (SIGTERM y, a los 5 s, SIGKILL) y avisa cuáles abortó; un hijo no puede anidar subagentes. Con esta tool `sdd-spec` ofrece `sdd-run con subagente`, `sdd-run` ofrece `Code review` sin GHA y `sdd-review-loop` corre en Pi. |
...
├── agents/         # agentes del plugin de Claude Code (~/.claude/agents)
...
│   └── subagent/   # tool `subagent` + agentes bundleados implementer/reviewer/scout
...
El plugin expone todos los skills de `claude/` y los agentes de `agents/` — los subagentes custom `implementer` y `reviewer` que la doctrina SDD invoca por su `description`, no por nombre fijo.

**Actualizaciones:** Claude Code no refresca solo los marketplaces de terceros: vienen con auto-update desactivado, así que el plugin queda en el commit que instalaste hasta que lo actualices a mano. Para que siga cada push a `main`, activá el auto-update del marketplace `chichex` de una de estas dos formas:

- en una sesión: `/plugin` → Marketplaces → `chichex` → **Enable auto-update**;
- o declarándolo en `~/.claude/settings.json`:
...
Destinos por defecto: `${CODEX_HOME:-~/.codex}/skills/`, `~/.claude/skills/`, `~/.claude/agents/`, `~/.config/opencode/skills/`, `~/.agents/skills/`, `~/.pi/agent/extensions/` y `~/.pi/agent/themes/` (overridables con `CODEX_SKILLS_DIR`, `CLAUDE_SKILLS_DIR`, `CLAUDE_AGENTS_DIR`, `OPENCODE_SKILLS_DIR`, `PI_SKILLS_DIR`, `PI_EXTENSIONS_DIR` y `PI_THEMES_DIR`). `CLAUDE_PLUGIN_REGISTRY_FILE` no es un destino: apunta al registro de plugins que `install.sh claude` lee para avisar si el plugin `chichex-skills` ya está instalado (y en ese caso saltear la instalación de `agents/`, que quedaría tapada por el plugin).
