...
| **`sdd-run`** | execution | Executes a spec end to end in an isolated worktree from the updated base. The original checkout may remain dirty: only the spec, contract, handoff, and other input artifacts tied to that workflow chain are imported into the branch, while everything else stays excluded and untouched. It then plans against the real code, implements **tests first**, verifies each criterion with its declared mechanism, and finishes in a **PR** with the spec as the body + evidence. |
| **`sdd-review-loop`** | PR polish | Chains up to N autonomous review-and-fix rounds on an existing PR: each round a `reviewer` subagent runs Claude Code's built-in `/code-review` with `--comment` (on Pi, the package's `code-review` skill, which posts a `COMMENT` review by default), and if correctness findings remain another `implementer` subagent fixes them following `sdd-run`'s remediation doctrine, verifies with the contract's commands (`.sdd/project.md`), pushes to the PR branch and resolves the addressed threads. The orchestrator stays in the main conversation and keeps only per-round counts and finding keys. It stops early when nothing is actionable, on non-convergence, or at the hard cap of 5 rounds, and leaves a single idempotent summary comment on the PR. Invoked bare it opens a wizard; with only `<PR>` it still asks for rounds, level, scope and models; any flag (`/sdd-review-loop <PR> [--rounds N] [--level L] [--fix-scope S] [--model M]`) skips it. Claude Code and Pi (on Pi `/skill:sdd-review-loop`, without `--level`: its `code-review` has no levels, and models are inherited from the session unless `--model`): requires `.sdd/project.md` and an open PR from the same repo. |
...
| **`subagent`** | The `subagent` tool delegates tasks to subagents with isolated context (`pi --mode json -p --no-session` children): single, parallel (up to 8 tasks, 4 concurrent) and chain (with `{previous}`) modes, `agentScope` `package`/`user`/`project`/`all` (default `package`: the bundled `implementer`, `reviewer` and `scout` agents in `pi-extensions/subagent/agents/`) and `background: true` to receive the result as a `subagent-result` message without blocking the session. The `/subagents` command lists jobs (`sa-N`, agent, status, duration) and `/subagents abort <id>` kills one. When the session closes it kills live children (SIGTERM, then SIGKILL after 5 s) and reports which ones were aborted; a child cannot nest subagents. With this tool `sdd-spec` offers `sdd-run con subagente`, `sdd-run` offers `Code review` without the GHA and `sdd-review-loop` runs on Pi. |
...
│   └── subagent/   # `subagent` tool + bundled implementer/reviewer/scout agents
...
├── evals/          # Claude Code plugin evals (claude plugin eval)
