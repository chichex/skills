...
| **`sdd-review-loop`** | PR polish | Chains up to N autonomous review-and-fix rounds on an existing PR: each round a Sonnet subagent runs Claude Code's built-in `/code-review` with `--comment`, and if correctness findings remain another subagent fixes them following `sdd-run`'s remediation doctrine, verifies with the contract's commands (`.sdd/project.md`), pushes to the PR branch and resolves the addressed threads. The orchestrator stays in the main conversation and keeps only per-round counts and finding keys. It stops early when nothing is actionable, on non-convergence, or at the hard cap of 5 rounds, and leaves a single idempotent summary comment on the PR. Invoked bare it opens a wizard; with only `<PR>` it still asks for rounds, level, scope and models; any flag (`/sdd-review-loop <PR> [--rounds N] [--level L] [--fix-scope S] [--model M]`) skips it. Claude Code only: requires `.sdd/project.md` and an open PR from the same repo. |
...
| **`coding-policies`** | Generates `.sdd/coding-policies.md` (or the path you ask for) in the project: a self-contained file with your coding best practices per stack — short MUST/SHOULD rules with their rationale and gate — detecting the repo's stacks (Go, TypeScript, Node, React, React Native, Kotlin Android) and linking it, with your confirmation, from `CLAUDE.md`, `AGENTS.md` or `.sdd/project.md`. Regenerating rewrites everything except the "Ajustes de este proyecto" section (project adjustments), which is preserved verbatim. It ships content for Go and TypeScript; other stacks are detected but reported as "sin prácticas definidas todavía". `sdd-init` references it as a `guia` when it exists and offers to generate it otherwise. Available in all four harnesses. |
