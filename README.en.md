...
| **`wait-pr`** *(Codex/Pi/opencode)* | Polls the current repository every 60 seconds for new PRs and runs `code-review` on each one sequentially without duplicates. It ignores already-open PRs unless `--include-open`, can stop after one with `--once`, and never inherits permission to publish comments. |
