...
## Stack
Repositorio de skills Markdown para cuatro harnesses — Claude Code (`claude/`), Codex (`codex/`), opencode (`opencode/`) y Pi (`pi/`) — mas extensiones TypeScript interpretadas por jiti y un Pi Package Git nativo declarado en el `package.json` raiz. El manifest es privado, ESM, sin scripts de publicacion, lockfile ni dependencias runtime; declara como peers las APIs core que Pi ya provee. Los tests usan `node:test` sobre TypeScript nativo y el gate `pi-extensions/pi-package/pi-package.test.ts` prueba inventario, carga RPC/theme, lifecycle y limpieza con configuracion descartable. Scripts de chequeo en `scripts/` y CI en `.github/workflows/ci.yml`; no hay build, `tsconfig` ni package manager local requerido. Verificado localmente con Node `v26.4.0`, Pi `0.85.1`, Bash `3.2.57`, Git `2.50.1` y GitHub CLI `2.96.0`. La extension `pi-extensions/subagent/` (tool `subagent`, comando `/subagents`, agentes bundleados `implementer`/`reviewer`/`scout`) es una adaptacion propia del ejemplo `examples/extensions/subagent` de Pi 0.85.1.

## Comandos
| Accion | Comando | cwd | Estado | Duracion | Notas |
|---|---|---|---|---|---|
| gate focalizado de Pi Package | `node --test pi-extensions/pi-package/pi-package.test.ts` | raiz | verificado 2026-09-22 | 8.5s | 18/18: metadata/peers (derivados de los imports reales via parser string-aware, no de una lista hardcodeada ni inyectados en el test; cinco peers, incluido `@earendil-works/pi-agent-core`), censo anti-drift sobre archivos trackeados en git (con test end-to-end de un scratch file untracked real), RPC con 22 comandos (incluidos `subagents` y `skill:sdd-review-loop`), theme, install/list/remove, limpieza (incluye nombres renombrados/eliminados upstream via manifest, y reconciliacion del bloque Codex), refuerzo de `install.sh pi` ante Pi Package nativo ya instalado, copia y limpieza de `pi-extensions/subagent/` entero (incluido `agents/`), y docs. Los 2 tests que invocan el binario `pi` (RPC/lifecycle) se skipean solos si `pi` no esta en PATH en vez de fallar |
| tests de extensiones | `node --test pi-extensions/*/*.test.ts` | raiz | verificado 2026-09-22 | 12s | 363/363; glob bloqueante de CI, incluye el gate de Pi Package, los gates SDD y el gate de la extension `subagent` |
| gate de la extension subagent | `node --test pi-extensions/subagent/*.test.ts` | raiz | verificado 2026-09-22 | <2s | 26/26; incluido tambien en la suite completa; prueba el descubrimiento de agentes por scope (`package` relativo a la extension, `user`, `project`, `all` con precedencia project > user > package), los args exactos del hijo `pi --mode json -p --no-session` con `spawn` inyectado (modelo/thinking de la sesion, `--tools`, prompt temporal 0600 borrado al terminar, task literal, `PI_SUBAGENT_DEPTH=1`), el rechazo de anidamiento, el modo background con `subagent-result` (`deliverAs: followUp`, `triggerTurn`, cap 50 KB), el kill SIGTERM/SIGKILL idempotente en `session_shutdown` y `/subagents`; el test de la factory real corre en un sandbox con los peers de la instalacion local de Pi y se skipea sin `pi` en PATH |
...
| gate anti-drift de agentes de plugin | `node --test pi-extensions/agents-gate/agents-gate.test.ts` | raiz | verificado 2026-09-22 | <1s | 33/33; incluido tambien en la suite completa; censa `agents/*.md` sobre archivos trackeados en git contra `implementer.md`/`reviewer.md` y `pi-extensions/subagent/agents/*.md` contra `implementer.md`/`reviewer.md`/`scout.md` (el unico port de `agents/`, con los mismos patrones ordenados de doctrina y frontmatter Pi: sin `model` ni `skills`, `tools` exacto en `reviewer`/`scout`), valida frontmatter (incluida la exigencia de `description` entre comillas dobles) y doctrina de `claude/sdd-review-loop/SKILL.md` y `claude/sdd-run/SKILL.md`, exige que `plugin.json` no declare `agents` (Claude Code solo acepta rutas `.md` ahi y el campo apaga el autodescubrimiento de `agents/`; `"./agents"` rompia la carga con `agents.0: Invalid input`), prueba `install.sh claude`/`all`/`both` con `CLAUDE_SKILLS_DIR`/`CLAUDE_AGENTS_DIR`/`CLAUDE_PLUGIN_REGISTRY_FILE` temporales (comparando mtime del HOME real antes/despues), y falla si aparece un `agents/` bajo `claude/`, `opencode/` o `pi/` a cualquier profundidad |
| lint de frontmatter | `bash scripts/lint-frontmatter.sh` | raiz | verificado 2026-09-22 | <1s | 57 skills OK; bloqueante en CI |
...
| smoke legacy de entrypoints | `args=(); for extension in pi-extensions/*.ts pi-extensions/*/index.ts; do [ -f "$extension" ] && args+=(--extension "$extension"); done; pi "${args[@]}" --list-models --offline` | raiz (Bash) | verificado 2026-08-15 | <2s | carga los 15 candidatos del glob legacy sin sesion/provider (incluidos helpers sin factory como `sdd-artifacts` y `workflow-resolution`); el gate del package selecciona y prueba exactamente 11 factories de produccion |
...
3. **Gate determinista del package:** `node --test pi-extensions/pi-package/pi-package.test.ts` compara el manifest contra el arbol real (via `git ls-files`, no el working tree) y prueba metadata no publicable, peers derivados de los imports reales de codigo trackeado, 15 skills, 11 factories y un theme. Sus autotests demuestran diagnosticos ante drift, metadata debilitada, imports externos sin peer declarado, y estilos de default-export mas alla de `export default function`.
...
6. **CI remota:** replica la estatica/suite, instala Pi 0.85.1 para los probes y agrega `shellcheck`.
...
## Gaps
- Pi 0.85.1 es la version contractual probada, no una version minima declarada del package.
- La tool `subagent` bloquea el anidamiento: el hijo hereda `PI_SUBAGENT_DEPTH=1` y la tool rechaza lanzar desde un proceso con esa marca. Un `pi` interactivo abierto a mano desde un hijo tambien queda bloqueado; aceptable y documentado.
- Los jobs de `subagent` viven en memoria por sesion: `/new` y `/reload` disparan `session_shutdown`, que mata los hijos vivos (SIGTERM, SIGKILL a los 5 s) y avisa cuales aborto; un run en curso se pierde y no hay persistencia de resultados fuera de la sesion.
- `--tools` filtra tambien las tools de extension: `reviewer` y `scout` (`tools: read, grep, find, ls, bash`) no ven `ask_user_question`, `subagent` ni `persist_sdd_spec`. Es deseado; `implementer` no declara `tools` y hereda todas.
- `pi-extensions/subagent/` vendoriza y adapta el ejemplo `examples/extensions/subagent` de Pi 0.85.1: cambios futuros del ejemplo upstream no llegan solos. La conducta real en TUI (background, `subagent-result`, `/subagents abort`, kill al cerrar) queda como prueba humana: exige provider y sesion interactiva.
