...
| tests de extensiones | `node --test pi-extensions/*/*.test.ts` | raiz | verificado 2026-09-17 | 4.5s | 317/317; glob bloqueante de CI, incluye el gate de Pi Package y los gates SDD |
...
| gate anti-drift de agentes de plugin | `node --test pi-extensions/agents-gate/agents-gate.test.ts` | raiz | verificado 2026-09-17 | <1s | 24/24; incluido tambien en la suite completa; censa `agents/*.md` sobre archivos trackeados en git contra `implementer.md`/`reviewer.md`, valida frontmatter (incluida la exigencia de `description` entre comillas dobles) y doctrina de `claude/sdd-review-loop/SKILL.md` y `claude/sdd-run/SKILL.md`, exige `"agents": ["./agents"]` en `plugin.json`, prueba `install.sh claude`/`all`/`both` con `CLAUDE_SKILLS_DIR`/`CLAUDE_AGENTS_DIR`/`CLAUDE_PLUGIN_REGISTRY_FILE` temporales (comparando mtime del HOME real antes/despues), y falla si aparece un `agents/` bajo `claude/`, `opencode/` o `pi/` a cualquier profundidad |
