...
| gate anti-drift de agentes de plugin | `node --test pi-extensions/agents-gate/agents-gate.test.ts` | raiz | verificado 2026-09-17 | <1s | 20/20; incluido tambien en la suite completa; censa `agents/*.md` sobre archivos trackeados en git contra `implementer.md`/`reviewer.md`, valida frontmatter y doctrina de `claude/sdd-review-loop/SKILL.md` y `claude/sdd-run/SKILL.md`, exige `"agents": ["./agents"]` en `plugin.json`, prueba `install.sh claude`/`all` con `CLAUDE_SKILLS_DIR`/`CLAUDE_AGENTS_DIR`/`CLAUDE_PLUGIN_REGISTRY_FILE` temporales, y falla si aparece un `agents/` bajo `claude/`, `opencode/` o `pi/` |
