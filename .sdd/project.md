...
| evals del plugin (Claude Code) | `claude plugin eval . --ablation none --runs 1 --max-cost-usd 5 --no-publish --allow-tools Write` | raiz | verificado 2026-09-22 | COSTO_SMOKE_DURACION | 8 casos en `evals/` (7 de triggering con graders `tool_used` gratis, 1 de adherencia de `sdd-spec` con fixture `evals/adherence-sdd-spec/fixtures/mini-cli` y rúbrica `llm`), `model: claude-sonnet-5` en cada caso; corrida única de calibración: COSTO_SMOKE_USD; sin TTY sumar `--trust-plugin`; a demanda `--case <glob> --runs 3 --ablation with-without`; en CI a pedido con `gh workflow run plugin-eval.yml` (secret `CLAUDE_CODE_OAUTH_TOKEN`). Paga y no determinista: no reemplaza los gates de `node --test` |
