...
| CA-10 | verificado | `node --test pi-extensions/*/*.test.ts`: 194/194; harness gate 25/25; frontmatter 47/47; `bash -n`, drift (63 líneas), smoke legacy (13 candidatos), `git diff --check` y diff contra base verdes. `gh pr checks 22 --watch`: 5/5 pass sobre `55af301`, incluido `Shell (bash -n + shellcheck)` (10s). |
