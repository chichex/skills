---
name: wait-pr
description: Monitorea el repositorio GitHub actual en busca de PRs nuevos y ejecuta el skill code-review sobre cada uno de forma secuencial. Usar cuando el usuario pida esperar, vigilar o monitorear nuevos pull requests y revisarlos al aparecer; no publica reviews porque invoca code-review con `--no-publish`.
compatibility: Requiere un repositorio git, GitHub CLI (gh) autenticado, acceso de lectura al repositorio y el skill code-review instalado. El monitor fuerza `--no-publish` y no requiere permiso de escritura.
---

<!-- wait-pr-doctrine:start -->
Espera pull requests nuevos en el repositorio GitHub correspondiente al cwd y, cuando aparecen, les aplica `/skill:code-review` con `--no-publish`. El monitoreo es de solo lectura; revisar está autorizado por la invocación, publicar comments no.
...
- `--once`: termina después de que la primera review detectada complete su reporte sin publicación o falle de forma terminal.
...
3. Cargar el skill `/skill:code-review` leyendo completo `../code-review/SKILL.md` y ejecutar su doctrina con la URL canónica y `--no-publish`. Este skill no duplica ni resume la doctrina de `/skill:code-review`: mantiene sus tres ejes, verificaciones, severidades y preview exacta, pero fija su opt-out de publicación.
4. La invocación de `/skill:wait-pr` autoriza detectar y ejecutar la review de solo lectura. No autoriza publicar comments, aprobar, pedir cambios, responder, pushear, cerrar ni mergear. Pasar `--no-publish` es obligatorio aunque `/skill:code-review` publique por default.
5. Esperar a que la review termine su reporte y limpieza antes de tomar la siguiente cola o volver al polling. Si el head cambia durante el análisis, registrar la limitación; no publicar ni fingir que se revisó el SHA nuevo.
...
- Delegar cada review a `/skill:code-review` mediante su SKILL.md real, pasando siempre `--no-publish`.
...
- No omitir `--no-publish` ni ejecutar ningún side effect de GitHub.
