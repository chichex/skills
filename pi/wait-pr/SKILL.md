---
name: wait-pr
description: Monitorea el repositorio GitHub actual en busca de PRs nuevos y ejecuta el skill code-review sobre cada uno de forma secuencial. Usar cuando el usuario pida esperar, vigilar o monitorear nuevos pull requests y revisarlos al aparecer; no publica reviews sin el gate explícito de code-review.
compatibility: Requiere un repositorio git, GitHub CLI (gh) autenticado, acceso de lectura al repositorio y el skill code-review instalado. Publicar comments conserva el gate y los permisos de code-review.
---

<!-- wait-pr-doctrine:start -->
Espera pull requests nuevos en el repositorio GitHub correspondiente al cwd y, cuando aparecen, les aplica `/skill:code-review`. El monitoreo es de solo lectura; revisar está autorizado por la invocación, publicar comments no.

## Argumentos

```text
/skill:wait-pr [--include-open] [--once]
```
...
- `--once`: termina después de que la primera review detectada resuelva el gate de publicación de `/skill:code-review` o falle de forma terminal.
...
2. Verificar antes de esperar que el skill `/skill:code-review` está disponible y que se puede leer `../code-review/SKILL.md`. Si falta o es ambiguo, frenar: no reemplazarlo con una review improvisada.
...
3. Cargar el skill `/skill:code-review` leyendo completo `../code-review/SKILL.md` y ejecutar su doctrina con la URL canónica como argumento. Este skill no duplica ni resume la doctrina de `/skill:code-review`: mantiene sus tres ejes, verificaciones, severidades, preview exacta y gate obligatorio.
4. La invocación de `/skill:wait-pr` autoriza detectar y ejecutar la review de solo lectura. No autoriza publicar comments, aprobar, pedir cambios, responder, pushear, cerrar ni mergear. El gate de `/skill:code-review` exige la misma confirmación explícita posterior de siempre.
5. Esperar a que la review llegue a su reporte y a que el usuario resuelva su gate antes de tomar la siguiente cola o volver al polling. Si el head cambia, rige la protección stale de `/skill:code-review`; no publicar ni fingir que se revisó el SHA nuevo.
6. Si la review falla antes del reporte, registrar el diagnóstico para ese PR y continuar con el siguiente salvo que la causa sea global —por ejemplo auth perdida, repo incorrecto o skill ausente—, que detiene todo el monitor.
...
- Delegar cada review a `/skill:code-review` mediante su SKILL.md real y conservar intacto su gate de publicación.
...
- No improvisar una review si `/skill:code-review` falta ni duplicar su doctrina dentro de este skill.
- No publicar comments ni ejecutar ningún side effect de GitHub sin la confirmación explícita que exige `/skill:code-review`.
