...
Después, escribí `~/.herdr-detach/<nombre>/handle.json` con el pedido original, nombre, `pane_id`, `workspace_id`, `cwd` y kind — es lo que hace posible el modo pelado. Cerrá con tres líneas: qué agente quedó, dónde, y que `/skill:herdr-detach` pelado muestra el estado.
