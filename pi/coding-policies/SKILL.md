...
description: Genera en el proyecto un archivo Markdown autocontenido con tus buenas prácticas por stack (v1 con contenido para Go) y lo engancha en CLAUDE.md, AGENTS.md o .sdd/project.md para que toda sesión las siga. Usar cuando el usuario pida "coding policies", "buenas prácticas", "políticas de código", "mis prácticas de Go", que el agente siga sus prácticas en este repo, o que genere o regenere .sdd/coding-policies.md. También cuando sdd-init lo encadene al terminar el contrato.
compatibility: Requiere las tools ask_user_question y ask_user_questions de la extensión Pi de este repo, más read/bash/edit/write para detectar stacks, generar el archivo y editar los archivos de contexto.
---
...
En Pi, las dos preguntas del lanzador (stacks y destino) pueden agruparse en una ronda de `ask_user_questions`; el resto de los gates usa `ask_user_question`.
...
- Stacks posicionales (`go`, `node`, `react`, `react-native`, `kotlin-android`) — saltean la confirmación de stacks de la Fase 0: se usan tal cual, aunque el marcador no se detecte (el usuario sabe más que el marcador). Un id desconocido frena con la lista de ids válidos.
...
## Fase 0 — Lanzador (solo con `/skill:coding-policies` pelado)
...
```text
/skill:coding-policies genera en este repo un archivo con tus buenas practicas por stack,
...
<!-- Generado por /skill:coding-policies el <YYYY-MM-DD>. Regenerar con /skill:coding-policies; "Ajustes de este proyecto" se preserva. -->
...
2. Avisar que todo salvo `## Ajustes de este proyecto` se reescribe, y usar `ask_user_question`: `Regenerar (Recomendado)` / `Cancelar`.
...
Detectar en la raíz `CLAUDE.md`, `AGENTS.md` y `.sdd/project.md`. Solo se ofrecen los que existen: el skill nunca crea archivos de contexto; si no existe ninguno, lo informa y termina. Luego usar `ask_user_question` con `selectionMode: "multiple"` — "¿Dónde agrego la referencia?": una opción por archivo existente. Editar únicamente los confirmados, de forma idempotente y verificando antes que `<ruta>` no esté ya presente (si ya está, se reporta `ya estaba` y no se toca):
...
3. **.sdd/project.md** — en la tabla de `## Politicas de generacion`, la fila `| coding-policies | <ruta> (<stacks>) | guia — sin gate: /skill:sdd-run la sigue al generar, la juzga el reviewer |`, reemplazando el sentinel "Sin politicas activas" por la tabla (`| Politica | Valor | Gate |`) si está, actualizando la fila en su lugar si ya existe, y sin tocar nada si la sección no existe: lo informa y sugiere `/skill:sdd-init --update`.
