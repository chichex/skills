...
Un envelope ausente, doble, extra, conflictivo o de otro proyecto falla cerrado antes de mutar Git. Estos handoffs **no reemplazan ni saltean** ninguna precondición de este skill: contrato, estado de la spec, base actualizada, importación controlada de artefactos, worktree, gate del plan, tests y verificación completa siguen siendo obligatorios. Sólo saltean la Fase 0 porque el usuario ya eligió el target y autorizó el launcher. Para una spec artefacto con `issue=null`, conservá esa identidad: no inventes ni crees un issue.
