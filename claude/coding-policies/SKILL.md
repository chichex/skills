...
```text
/coding-policies [go|node|react|react-native|kotlin-android ...] [--out <ruta>] [--no-link]
```
...
2. Avisar que todo salvo `## Ajustes de este proyecto` se reescribe, y usar `AskUserQuestion`: `Regenerar (Recomendado)` / `Cancelar`.
