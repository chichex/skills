...
2. **Oferta después del cierre**: solo después de emitir `Run completo` y únicamente si hay un PR creado, usar `AskUserQuestion`: `Esperar comentarios (Recomendado)` — quedarse vigilando ese PR — / `Terminar` — cerrar la sesión. Con `--assume`, no preguntar y no esperar. Tampoco ofrecer la espera con `--no-pr`, degradación a commit local o `RUN INTERRUMPIDO`.
