...
En Claude Code, las opciones del menú van en un `AskUserQuestion` autocontenido, con el link o la ruta en la pregunta. `sdd-run` carga el skill `sdd-run` sobre el target en esta sesión. `sdd-run con subagente` lanza la tool `Agent` con `subagent_type: "implementer"` y `run_in_background: true`, con un prompt que le pide seguir el skill `sdd-run` sobre el target (`/sdd-run <target> --assume`) y devolver el PR y el reporte; si el tipo `implementer` no está disponible en la instalación, usar `general-purpose` y anunciarlo. La sesión queda libre: al llegar la notificación del subagente, relevar el PR y el reporte al usuario.
