...
La review termina primero en pantalla. Despues muestra una preview exacta y usa `request_user_input` para decidir si publicar los comments en GitHub. La opcion segura por default es no publicar.
...
- Sin argumento: intentar resolver el PR abierto de la branch actual con `gh pr view`. Si no hay uno inequivoco, pedir numero o URL con `request_user_input`.
...
- Pedir confirmacion explicita con `request_user_input` antes de cualquier escritura en GitHub.
