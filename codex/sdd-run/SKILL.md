...
Planificar contra el codigo real, no contra la idea del codigo. Explorar con lectura de archivos y shell y llamadas paralelas solo para comprobaciones independientes; Codex conserva el ownership en el agente principal:
...
**Gate**: presentar el plan resumido (pasos ↔ CAs, archivos que toca, que queda explicitamente afuera) y usar `request_user_input`: `Aprobar (Recomendado)` / `Ajustar` (el usuario dice que via custom y se replantea). Con `--assume`: sin gate. El plan NO se escribe a disco — vive en la conversacion y muere con ella.
