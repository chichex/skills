...
Si hay una sola spec candidata, usarla directo informando cuál. Con varias, mostrar "¿Cuál spec corremos?": una opción por spec (máximo 4, las más recientes) + `Ninguna, hay que especificar primero` → ofrecer `$sdd-spec`. Como puede haber más de tres opciones, pedir la elección en texto plano.
