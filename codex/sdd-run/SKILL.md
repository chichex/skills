...
Luego mostrar "¿Cuál spec corremos?": una opción por spec (máximo 4, las más recientes) + `Ninguna, hay que especificar primero` → ofrecer `$sdd-spec`. Como puede haber más de tres opciones, pedir la elección en texto plano.
...
- El plan declara los **seams** bajo prueba — las interfaces públicas donde se observa comportamiento. Preferir seams existentes y el más alto posible; el gate del plan es donde el usuario los aprueba.
...
**Gate**: presentar el plan resumido (pasos ↔ CAs, archivos que toca, qué queda explícitamente afuera) y usar `request_user_input`: `Aprobar (Recomendado)` / `Ajustar` (el usuario dice qué vía custom y se replantea). Con `--assume`: sin gate. El plan NO se escribe a disco — vive en la conversación y muere con ella.

## Fase 3 — Implementar con loop de verificación por CA

1. **Tests primero** (CA ALTA): escribir los tests del plan de verificación — solo en los seams aprobados en el gate, comportamiento por interfaces públicas, mocks solo en límites de sistema, nunca tautológicos — correrlos y confirmar que fallan por la razón correcta. Recién después implementar hasta verde.
...
2. **Oferta después del cierre**: solo después de emitir `Run completo` y únicamente si hay un PR creado, usar `request_user_input` cuando esté disponible y, si no, mostrar el mismo gate en texto plano y terminar el turno: `Esperar comentarios (Recomendado)` — quedarse vigilando ese PR — / `Terminar` — cerrar la sesión. Con `--assume`, no preguntar y no esperar. Tampoco ofrecer la espera con `--no-pr`, degradación a commit local o `RUN INTERRUMPIDO`.
