...
- **Pedido libre o grill sin issue de origen** — usar `request_user_input`: 1. `Local (Recomendado)` — `.sdd/specs/<slug>.md`; 2. `Crear issue` — crear primero el issue con un body de staging no-SDD; con el número devuelto, reemplazar `issue=none` y recién entonces actualizar el body con la spec canónica.
