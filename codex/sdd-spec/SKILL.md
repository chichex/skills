...
En Codex, el menú final usa `request_user_input` cuando tiene 2-3 opciones (Codex no tiene subagentes, así que `sdd-run con subagente` se omite y el menú queda en 3 o 2 opciones); si no está disponible, preguntar en texto plano, terminar el turno y continuar tras la respuesta. `sdd-run` carga y ejecuta el skill `$sdd-run` sobre el target en esta sesión.
