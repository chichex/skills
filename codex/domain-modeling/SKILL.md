...
Si el repo no tiene `CONTEXT.md`, `CONTEXT-MAP.md` ni `docs/adr/`, solo podés crear esos artefactos cuando el usuario lo pidió explícitamente en la conversación. Invocar `$domain-modeling` o elegir **Grill + documentación de dominio** dentro de `$grill` cuenta como pedido explícito. Invocar un grill estándar no cuenta como consentimiento.
...
En Codex, usá `request_user_input` para esa aprobación solo cuando esté disponible. Si no, mostrale el borrador, pedí el OK en texto plano y terminá el turno. Resolver una decisión por vez.
