...
Si hay una sola spec candidata, usarla directo informando cuál. Con varias, usar `question` — "¿Cuál spec corremos?": una opción por spec (máximo 4, las más recientes; el resto vía custom) + `Ninguna, hay que especificar primero` → ofrecer `/sdd-spec`.
